...
namespace ART_ROWEX {
    ...
    void N::insertGrow(curN *n, N *parentNode, uint8_t keyParent, uint8_t key, N *val, ThreadInfo &threadInfo, bool &needRestart) {
	    /* instead of undologging in "n" in the insertandunlock function
	     * above we should do this inside each insert function anyways if
	     * the n->insert below fails a new copy will be made, so we can just
		* skip it*/
        if (n->insert(key, val)) {
            n->writeUnlock();
            return;
	...
	ulog_add(node, sizeof(*node), node->clk, NULL);
        switch (node->getType()) {
            case NTypes::N4: {
                auto n = static_cast<N4 *>(node);
